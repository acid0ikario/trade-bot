...
def test_top_quartile_and_recommendations(tmp_path: Path):
    ...
    df = pd.DataFrame(
        [
            {"ema_fast": 10, "ema_slow": 20, "rsi_period": 14, "rsi_buy_min": 40, "rsi_buy_max": 60, "atr_k": 1.5, "risk_rr": 2.0, "sharpe": 1.0, "max_dd": -0.1, "pf": 1.2, "cagr": 0.1, "n_trades": 60},
            {"ema_fast": 12, "ema_slow": 24, "rsi_period": 14, "rsi_buy_min": 45, "rsi_buy_max": 60, "atr_k": 1.2, "risk_rr": 2.2, "sharpe": 2.0, "max_dd": -0.05, "pf": 1.4, "cagr": 0.2, "n_trades": 55},
            {"ema_fast": 8,  "ema_slow": 16, "rsi_period": 14, "rsi_buy_min": 35, "rsi_buy_max": 55, "atr_k": 1.8, "risk_rr": 1.8, "sharpe": 0.5, "max_dd": -0.2, "pf": 0.9, "cagr": -0.1, "n_trades": 20},
            {"ema_fast": 11, "ema_slow": 22, "rsi_period": 14, "rsi_buy_min": 42, "rsi_buy_max": 58, "atr_k": 1.4, "risk_rr": 2.1, "sharpe": 1.8, "max_dd": -0.08, "pf": 1.1, "cagr": 0.15, "n_trades": 50},
        ]
    )
    ...
    rec = optimize_from_csv(csv_path, out_yaml, pf_min=1.0, cagr_min=0.0, max_dd_max=0.40, ntrades_min=50)
...
def test_ab_outputs(tmp_path: Path, monkeypatch):
    ...
    def fake_run_backtest(symbol, timeframe, years, cfg, param_grid, data_loader=None):
        ...
        return [{"sharpe": base, "max_dd": -0.1 + 0.01 * base, "winrate": 0.5 + 0.1 * base, "pf": 1.5 + 0.2 * base, "expectancy": 0.01 * base, "cagr": 0.1 * base, "n_trades": 42}]
    ...
    assert set(["variant", "sharpe", "max_dd", "winrate", "pf", "expectancy", "cagr", "n_trades"]).issubset(set(out.columns))
